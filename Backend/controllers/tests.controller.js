...
const getAllQuestions = async (req, res) => {
  ...
  return res.status(200).json(
    new customApiResponse("Sucessfully retrived mock questions !!", 200, {
      allQuestions,
    }),
  );
};
...
const getMathQuestions = asyncHandler(async (req, res) => {
  const allQuestions = await questionsModel
    .find({ category: "math" })
    .select("-correctAnswer");

  return res.status(200).json(
    new customApiResponse("Sucessfully retrived math questions !!", 200, {
      allQuestions,
    }),
  );
});
...
const getLogicalQuestions = async (req, res) => {
  const allQuestions = await questionsModel
    .find({ category: "logical" })
    .select("-correctAnswer");

  return res.status(200).json(
    new customApiResponse("Sucessfully retrived logical questions !!", 200, {
      allQuestions,
    }),
  );
};
...
const getComputerQuestions = async (req, res) => {
  const allQuestions = await questionsModel
    .find({ category: "computer" })
    .select("-correctAnswer");

  return res.status(200).json(
    new customApiResponse("Sucessfully retrived computer questions !!", 200, {
      allQuestions,
    }),
  );
};
...
const submitTest = asyncHandler(async (req, res) => {
  const { answers, startTime, testId, testCategory } = req.body;
  ...
  if (!currentUser){
    throw new CustomApiError({message: "You cannot submit the test unless you logged in"}, {status: 304})
  }
  try {

    const newTest = await testsModel.create({
